...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO scholar_certificate_of_registration (report_title, submission_date, report_content, file, report_month) 
        ...
        if ($stmt->execute()) {
            echo "<script>
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Certificate of Registration submitted successfully!',
                    confirmButtonColor: '#003c3c',
                    background: '#003c3c',
                    color: '#fff'
                }).then((result) => {
...
$submittedReports = [];
$stmt = $pdo->query("SELECT report_month FROM scholar_certificate_of_registration");
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="container shadow-lg rounded-4 pb-5" style="padding: 1rem 4rem 0; background: linear-gradient(125deg, #003c3c 0%, #004949 15%, #005555 30%, #007373 45%, #008080 60%, #00a6a6 80%, #00b5b5 100%); height: 95%;">
                <h3 class="fw-bold fs-1 text-center mb-2 text-white">Certificate of Registration</h3>
